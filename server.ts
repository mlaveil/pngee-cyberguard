...
async function startServer(){
 ...
 const app=express(),PORT=Number(process.env.PORT||3000);app.set('trust proxy',Number(process.env.TRUST_PROXY||0));app.disable('x-powered-by');app.use(securityHeaders);app.use(rejectInsecureProductionRequests);app.use(apiRateLimit);app.use(express.json({limit:'1mb'}));app.use(express.urlencoded({extended:true,limit:'1mb'}));app.use(cookieParser());
 ...
 app.use('/api/v1',agentRouter);app.use('/api',agentRouter);
 ...
 app.use('/api/v1',apiRouter);app.use('/api',apiRouter);
 setInterval(()=>{try{AgentService.checkHeartbeatTimeouts();}catch(err){console.error('[Watchdog] Error:',err);}},30000);
 if(process.env.NODE_ENV!=='production'){const vite=await createViteServer({server:{middlewareMode:true,host:'0.0.0.0',port:PORT},appType:'spa'});app.use(vite.middlewares);}else{const distPath=path.join(process.cwd(),'dist');app.use(express.static(distPath,{index:false}));app.get('*',(req,res)=>res.sendFile(path.join(distPath,'index.html')));}
