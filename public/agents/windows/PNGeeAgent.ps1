...
function Send-PNGeeTelemetry{param([string]$EndpointPath,[hashtable]$Payload)
...
function Run-AgentCycle{Write-Host '[*] Sending authenticated PNGee telemetry...' -ForegroundColor Cyan;Send-PNGeeTelemetry '/api/v1/telemetry/heartbeat' @{status='ONLINE';timestamp=(Get-Date).ToUniversalTime().ToString('o')}|Out-Null;Send-PNGeeTelemetry '/api/v1/telemetry/system' (Get-SystemMetrics)|Out-Null;Send-PNGeeTelemetry '/api/v1/telemetry/security' (Get-SecurityMetrics)|Out-Null;Send-PNGeeTelemetry '/api/v1/telemetry/services' @{services=(Get-CriticalServices);timestamp=(Get-Date).ToUniversalTime().ToString('o')}|Out-Null;Write-Host '[+] Telemetry cycle dispatched.' -ForegroundColor Green}
