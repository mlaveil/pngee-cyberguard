...
export async function authMiddleware(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    ...
    if (!row) {
      res.status(401).json({ error: 'Unauthorized', message: 'User is disabled or no longer exists.' });
      return;
    }
    ...
    if (!isGlobalAdmin) {
      if (requestedOrg && requestedOrg !== req.user.organizationId && requestedOrg !== 'current') {
...
export function requireRoles(allowedRoles: UserRole[]) {
  return (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    if (!expanded.has(req.user.role)) {
      res.status(403).json({ error: 'Forbidden', message: `Insufficient permissions. Requires one of: ${allowedRoles.join(', ')}` });
      return;
    }
    next();
  };
