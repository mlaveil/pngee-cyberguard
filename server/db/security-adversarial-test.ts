...
const TENANT_TABLES = ['users','assets','endpoint_identities','enrollment_tokens','security_events','alerts','incidents','audit_logs','detection_rules'];
...
async function main() {
  ...
  const eventA = crypto.randomUUID(), eventB = crypto.randomUUID();
  ...
  try {
    const inventory = await query<{ tablename: string; relrowsecurity: boolean; relforcerowsecurity: boolean }>(
      `SELECT c.relname AS tablename, c.relrowsecurity, c.relforcerowsecurity FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname='public' AND c.relkind='r' AND c.relname = ANY($1::text[])`, [TENANT_TABLES]);
    if (inventory.rows.length !== TENANT_TABLES.length) throw new Error('Tenant RLS inventory is incomplete');
    for (const row of inventory.rows) if (!row.relrowsecurity || !row.relforcerowsecurity) throw new Error(`RLS/FORCE RLS missing on ${row.tablename}`);
    ...
    await withSecurityContext(null, true, async client => {
      ...
      await client.query(`INSERT INTO security_events (id,organization_id,asset_id,event_type,severity,source,metadata,payload) VALUES ($1,$2,$3,'adversarial','HIGH','security-adversarial',$4,$5),($6,$7,$8,'adversarial','HIGH','security-adversarial',$9,$10)`, [eventA,orgA,assetA,JSON.stringify({canary:canaryA}),JSON.stringify({canary:canaryA}),eventB,orgB,assetB,JSON.stringify({canary:canaryB}),JSON.stringify({canary:canaryB})]);
    });
    ...
    const aEvents = await withSecurityContext(orgA,false,client => client.query(`SELECT id,metadata,payload FROM security_events WHERE id IN ($1,$2)`,[eventA,eventB]));
    ...
    await expectRejected('Tampered access token',() => verifyAccessToken(`${parts[0]}.${parts[1]}.${Buffer.from(parts[2]).toString('base64url')}`));
    await expectRejected('Unsecured JWT algorithm',() => verifyAccessToken(`${Buffer.from(JSON.stringify({alg:'none',typ:'JWT'})).toString('base64url')}.${parts[1]}.`));

    const mfa = await issueMfaChallenge(userA);
    if (await verifyMfaChallenge(mfa) !== userA) throw new Error('MFA challenge subject validation failed');
    const mfaParts = mfa.split('.');
    await expectRejected('Tampered MFA challenge',() => verifyMfaChallenge(`${mfaParts[0]}.${mfaParts[1]}.${Buffer.from(mfaParts[2]).toString('base64url')}`));

    console.log('Security adversarial tests: PASS');
  } finally {
    await withSecurityContext(null,true,async client => {
      await client.query('DELETE FROM security_events WHERE id IN ($1,$2)',[eventA,eventB]);
