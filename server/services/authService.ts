...
export async function issueMfaChallenge(userId: string) { return new SignJWT({ purpose: 'mfa' }).setProtectedHeader({ alg: 'HS256', typ: 'JWT' }).setSubject(userId).setIssuer(issuer).setAudience(audience).setIssuedAt().setExpirationTime('5m').sign(secret()); }
export async function verifyAccessToken(token: string) { return jwtVerify(token, secret(), { issuer, audience, algorithms: ['HS256'] }); }
export async function verifyMfaChallenge(token: string) { const verified = await jwtVerify(token, secret(), { issuer, audience, algorithms: ['HS256'] }); if (verified.payload.purpose !== 'mfa' || typeof verified.payload.sub !== 'string') throw new Error('Invalid MFA challenge'); return verified.payload.sub; }
